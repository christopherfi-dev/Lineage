...
The same moments as `design/current/` (the before), shot again after Step 5: part one, the beauty pass that brings the game up to `design/v2/`, and part two, family names, sound and animals that move like animals. Same links, same seed (6), same iPad sizes: landscape 1180 × 820 and portrait 820 × 1180, at 2× pixels, saved as JPEG.
...
| File | When | Why |
|---|---|---|
| `arrival-*.jpg` | 9 s after the page opens | The mist lifts over about 7 s; after it, the opening looks like the before shot. |
...
|---|---|---|
| Arrival | `arrival-*`, `arrival-mist-*` | Morning mist, the camera drifting down, then the hint and the log line. |
...
| A creature card | `card-*` | The card, its habitat scene, light along the animal's back and the ring under the new part. |

*Made on 2026-09-26 with headless Chromium from the links above (seed 6), and made again the same day after part two.*
