 ...
 * frame costs a handful of fills and a few hundred tiny drawImage calls.
 */
...
export class Sky {
  ...
  drawWorld(x, v, L, mood, now) {
    ...
    // Warm light from the sun's side of the screen.
    const g = x.createLinearGradient(L.sun < 0 ? v.x + v.w : v.x, v.y, L.sun < 0 ? v.x : v.x + v.w, v.y + v.h);
    g.addColorStop(0, rgb(L.lift)); g.addColorStop(1, "rgba(0,0,0,0)");
    x.globalCompositeOperation = "screen";
    x.globalAlpha = L.lA * (1 + 0.5 * Math.max(0, mood)) * (1 - 0.4 * Math.max(0, -mood));
    x.fillStyle = g; x.fillRect(v.x, v.y, v.w, v.h);
  ...
  drawAir(x, vw, vh, L, mood, now, mist, cam) {
    ...
    // A soft vignette, deeper at night and when the group shrinks.
    x.globalAlpha = 0.5 + 0.35 * L.night + 0.15 * Math.max(0, -mood);
