...
| **Following a trait** | `?moment=follow` | A glowing newborn's card with the fair test's real size, "Follow 20 animals with a weaker tail", and "Keep looking", which closes the card and leaves the glow on (scope decision 43; generation 11, after 1 follow). | `follow-*.jpg` · main.js (`showCard`, `renderFollow`, `keepLooking`), cohorts.js (`sidesIn`, `testSize`, `canStart`), narration.js (`followButton`, `KEEP_LOOKING`), styles.css (`#card-follow`) |
...
| **Wait!** | `?moment=danger` | A spread stopped because the child's group got very small (scope decision 44). Family 4, generation 47, before any follow: more curved claws were being fast-forwarded (9 → 13 → 8 → 10 → 6 → 8 with it) when the family fell to 2. The fast-forward stops at once: the Fast-forward badge is gone, the camera is on the family, and the log says "Wait! Your family is getting very small." ("family", since there was no follow yet). The world goes on at its usual pace. | `danger-*.jpg` · story.js (`DANGER_SIZE`, `inDanger`, `spreadGeneration`, `stopSpread`), main.js (`spreadDanger`), narration.js (`dangerLine`) |
| **Stay with them?** | `?moment=blocked` | The webbed family in the high leaves at generation 2, down to 4 animals. A glowing baby with shorter back legs is too rare there to start a fair test, and the family is too small for a spread, so its card says "Your family needs you. Stay with them?" with only "Keep looking" (scope decision 44). | `blocked-*.jpg` · main.js (`renderFollow`), story.js (`inDanger`, `trySpread`), narration.js (`needsYou`, `KEEP_LOOKING`), styles.css (`#card-follow .follow-note`) |
...
**follow**, **spreading**, **fizzled**, **danger**, **blocked**, **fairtest** and **choice** (now the backup panel) are active choosing (scope decisions 32–34, 42 and 44). **prediction** and **prediction-result** are the prediction journal (Step 6).

*All screenshots made again on 2026-09-24 with headless Chromium from the links above (seed 6), with active choosing, the adaptive fair-test size and the spread fast-forward (scope decisions 32–38 and 42). The ending and died-out shots were made again with the seventeen reveal animals and their facts (scope decisions 45 and 46), and the danger and blocked shots were added (scope decision 44).*
