...
| **Watching** | `?moment=watching` | A watched variation is common enough (10 on each side): "Your animals with thinner fur: now 11. Follow them?", with a "Follow them" button above the log. The corner panel lists what is watched, with counts. Generation 26, family 4. | `watching-*.jpg` · story.js (`watch`, `updateWatching`), main.js (`offerReady`, `updateHud`), narration.js (`readyLine`, `watchRow`), styles.css (`#ready`, `#hud #watching`) |
...
| **Died out** | `?moment=extinct` | Your group died out after 10 follows: "Their story lasted 60 generations." The ending leads with the fair test (scope decision 37): "In the high leaves: Yours (more curved claws): 13 → 0, The others here: 13 → 26", "The others here are still alive.", then the question. No reveal. | `extinct-*.jpg` · main.js (`showEnding`: `#ending-lead`), narration.js (`OTHERS_DIED_TOO`, `OTHERS_ALIVE`), styles.css (`#ending-lead`) |
...
*All screenshots made again on 2026-09-24 with headless Chromium from the links above (seed 6), with active choosing and the adaptive fair-test size (scope decisions 32–38).*
