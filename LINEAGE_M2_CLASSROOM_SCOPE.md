...
6. The story loop. A story follows one family through a series of choices. At each choice point the world pauses and the child chooses which animal to follow from two or three family members, each carrying a different variation that is already carried by at least 3 family members. The child chooses whom to follow, never what mutates; observer choices never affect the biology. If the child does not choose within CHOICE_SECONDS (start at 20), one option is picked at random and the screen says so. After a choice, the world fast-forwards SKIP_GENERATIONS (start at 10) visibly faster. A story ends when the followed line dies out, or after STORY_CHOICES (start at 5) choices. Every ending is a reflection screen, not a game-over screen. Surviving families will later be revealed as the real animal they most resemble; there is no single correct line.
